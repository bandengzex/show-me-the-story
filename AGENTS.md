...
- **前端**：Vite 5 + Svelte 4 + Tailwind CSS 3 + DaisyUI 4
...
| 文件 | 职责 |
|------|------|
| `package.json` | 前端依赖：Svelte 4、Vite 5、Tailwind CSS 3、DaisyUI 4 |
| `vite.config.js` | Vite 配置：Svelte 插件、dev server 代理 `/api` → `:48090`、构建输出到 `dist/` |
| `tailwind.config.js` | Tailwind + DaisyUI 配置：`night` 主题自定义色值 |
| `postcss.config.js` | PostCSS：Tailwind + Autoprefixer |
| `svelte.config.js` | Svelte 预处理器配置 |
| `index.html` | 入口 HTML，`data-theme="night"` |
| `src/main.js` | Svelte 应用挂载点 |
| `src/app.css` | 全局样式：Tailwind 指令 + 自定义滚动条/toast 动画 |
| `src/App.svelte` | 根组件：Header + 停止按钮 + 侧边导航 + 页面路由 + LogPanel + Toast 容器 |
| `src/lib/api.js` | `api(method, url, body)` — fetch 封装 |
| `src/lib/router.js` | `currentPage` store + hash 路由监听 |
| `src/lib/stores.js` | 全局 Svelte stores（progress、config、settings、taskRunning 等）+ toast/log 管理 |
| `src/lib/sse.js` | `connectSSE()` — EventSource 连接 + 12 种事件处理 → 更新 stores |
| `src/pages/Config.svelte` | 配置页：API 配置、故事配置、角色管理、世界观管理 |
| `src/pages/Outline.svelte` | 大纲页：生成/确认/修订/删除 + 导入续写 + 内联编辑 |
| `src/pages/Writing.svelte` | 写作页：章节列表/查看/生成/确认/修改/去AI味 + 进度条 + 流式显示 |
...
`tryStartTask()` 创建带 cancel 的 `context.Context`，`endTask()` 调用 cancel 释放资源。`PostTaskStop` handler 调用 `taskCancel()` 取消运行中的任务。
...
独立 `agent.go` 文件，`RunAgentLoop(goCtx context.Context, ctx *AgentContext, ...)` 函数实现工具调用循环，接受 `context.Context` 支持任务取消。内置工具集包括：`read_characters`、`read_character`、`read_worldview`、`read_organizations`、`read_chapter`、`read_outline`、`read_foreshadows`、`search_project`、`create_character`、`update_character`、`create_worldview`、`update_worldview`。仅全局助理使用 Agent Loop。
...
致命错误检测：`isFatalAPIError` 检测连接被拒绝、DNS 解析失败、HTTP 401/404 等不可恢复错误，立即停止重试。`validateAPIConfig` 在任务开始前检查 BaseURL 和 Model 是否为空。
...
前端使用 Vite 5 + Svelte 4 + Tailwind CSS 3 + DaisyUI 4 构建，产物输出到 `frontend/dist/`，通过 `//go:embed frontend/dist` 内嵌到 Go 二进制。

- **页面**：`config`（配置 + 角色管理 + 世界观管理）、`outline`（大纲，含导入功能）、`writing`（写作 + 去AI味按钮）、`relations`（关系图谱 Canvas）、`assistant`（全局助理聊天）、`skills`（技能管理）
- **状态管理**：Svelte stores（`src/lib/stores.js`），包含 progress、config、settings、taskRunning 等全局状态
- **路由**：hash 路由（`src/lib/router.js`），`currentPage` store + `window.hashchange` 监听
- **API 调用**：`api(method, url, body)` 封装 fetch（`src/lib/api.js`）
- **SSE**：`connectSSE()` 建立 EventSource 连接，12 种事件类型自动更新 stores（`src/lib/sse.js`）
